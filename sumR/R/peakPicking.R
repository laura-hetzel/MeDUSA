...
groupCells <- function(res){
  ...
  snrs <- matrix(nrow = nrow(x), ncol = length(res))

  for (i in 1:nrow(x)) {
    ids <- unlist(x[i, "peakidx"])
    m[i, df$sample[ids]] <- df$i[ids]
    snrs[i, df$sample[ids]] <- df$meanSNR[ids]
  }
  dimnames(m) <- list(rownames(x), names(res))
  x <- x[, -which(colnames(x) == "peakidx")]
  SummarizedExperiment(assays = list(Area = m, SNR = snrs), rowData = x)
}
...
#' @export
imputation <- function(exp){
  df <- log2(as.data.frame(assay(exp)))
  df[is.na(df)] <- 0
  assay(exp, "imputed") <- saver(as.matrix(df), estimates.only = T)
  exp
}
