#' Mydensity clust 1
#'
#' @description this is the function adapted from the densityClust() function
#' of the densityClust package. It returns the rho&delta decision graph and
#' the clusters it finds. mz need to cluster in intervals, otherwise the
#' data amount is too large to calculate/store
#'
#' @param data
#'
#' @param mz_min
...
cluster_align <- function(bin_element) {
  ...
  if (nrow(clusteredData) != 0) {
    ...
    ## get the cluster ID of which cluster is adjacent to another cluster only
    ## contains 1 peak
    if (length(tocheck) > 1) {
      v <- vector()
    ...
    for (i in seq_len(nrow(clusteredData))) {
  ...
  # tsfData <- tmpData %>% group_by(aligned) %>%
...
#' @description this is the function adapted from the densityClust() function
#' of the densityClust package. It returns the rho&delta decision graph and
#' the clusters it finds. The default rho = 0.8, delta = 0.05. These should be
#' tuned based on the decision graph for each data set.
#'
#' Cluster needs to be done in intervals, otherwise the data amount is too
#' large to calculate/store.
#'
#' @param data
#' @param mz_min
#' @param mz_max
...
#'
#' @examples
mydensityClust_2 <- function(data, mz_min, mz_max, dc) {
  ...
  # subset mz for testing
  tmz <- mz[mz > mz_min & mz < mz_max]

  # calculate the distance (in ppm) between each mz and put them in a data frame
  distMatrix <- matrix(data = NA, nrow = length(tmz), ncol = length(tmz))
  for (i in seq_along(tmz)) {
    for (j in seq_along(tmz)) {
      distMatrix[i, j] <- abs(ppm_calc(tmz[j], tmz[i]))
    }
  }
  distMatrix <- as.dist(distMatrix, diag = TRUE)
