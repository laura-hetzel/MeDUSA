...
```{r peakPicking}
peaks <- peakPicking(files, massDefect = FALSE, massWindow = 200, noiseWindow = 0.1, polarity = "-")
```
...
```{r spectraBinning}
spectra <- spectraBinning(peaks, npeaks = 5, method = "sum", tolerance = 0.002)
```
...
```{r cellBinning}
exp <- cellBinning(spectra, cellData = df, phenotype = "phenotype", tolerance = 0.002)
exp
```
...
```{r Blanks}
exp <- blankSubstraction(exp, blankThresh = 5, sampleThresh = 1,
...
```{r imoputation}
exp <- imputation(exp, method = "noise", noise = 1000, seed = 42)
exp
```
...
exp2 <- peakPicking(files, massDefect = FALSE, massWindow = 200, polarity = "+") %>%
  ...
  spectraBinning(npeaks = 5, method = "sum", tolerance = 0.002) %>%
  cellBinning(cellData = df, phenotype = "phenotype", tolerance = 0.002) %>%
  blankSubstraction(sampleThresh = 1) %>%
