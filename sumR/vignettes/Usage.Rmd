...
### Selecting scans
Due to switching polarity and fragmented scans. we need to create a filter for certain scans.  
```{r prepareScans}
fileList <- prepareScans(files, massWindow = c(100, 200), polarity = "+", centroid = F)
```
...
```{r peakPicking}
peaks <- peakPicking(files, noiseWindow = 0.1)
```
...
```{r cellBinning}
exp <- cellBinning(spectra, cellData = df, phenotype = "phenotype", tolerance = 5e-6)
exp
```
