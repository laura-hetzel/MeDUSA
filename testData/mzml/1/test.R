...
#mzL <- sqrlSumr::mzml_extract_magic(cores=4, magic=F)
#mzL_filter(threshold=2) #requires value in 2 Scans
#mzL_timeSquash(method="x^2", ignore_below = 100)

meta <- data.frame(read.csv("meta_neg.csv"))

mz_obj <-  mzL$neg

mz_carbon <- sqlrlSumr::mz_isotope_hunter(mz_obj, cores=4)
